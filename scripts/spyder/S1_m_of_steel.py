...
from scripts.spyder.utils import parse_json_from_llm_output

# Helper 1: Download the latest PDF from the steel.gov.in summary table (first data row)
def download_pdf_by_rank(url, rank, download_dir=None):
    """
    Download a PDF from the monthly summary table by row rank.

    :param url: The target page URL.
    :param rank: 1 = top row, 2 = second row, etc.
    :param download_dir: Directory to save the file. If None, saves to a temp file.
    """
    if rank < 1:
        raise ValueError("rank must be >= 1 (1 = top row)")
    ...
    if not data_rows:
        raise Exception("No data rows in tbody.")

    row_index = rank - 1
    if row_index >= len(data_rows):
        raise IndexError(
            f"Requested rank {rank} is out of bounds. "
            f"Table has {len(data_rows)} row(s)."
        )

    target_row = data_rows[row_index]

    docs_cell = target_row.select_one("td.views-field-field-documents")
    if not docs_cell:
        tds = target_row.find_all("td", recursive=False)
        docs_cell = tds[-1] if tds else None
    if not docs_cell:
        raise Exception(f"Could not find documents cell in row {rank}.")

    chosen_pdf_link = None
    for a in docs_cell.find_all("a", href=True):
        href = (a["href"] or "").strip()
        if href.lower().split("?")[0].endswith(".pdf"):
            chosen_pdf_link = href
            break

    if not chosen_pdf_link:
        raise Exception(f"No PDF link found in row {rank}.")

    target_pdf_url = urljoin(url, chosen_pdf_link)
    pdf_response = requests.get(target_pdf_url, verify=False)
    if pdf_response.status_code != 200:
        raise Exception(f"Failed to download PDF: {target_pdf_url}")

    filename = unquote(target_pdf_url.split("/")[-1].split("?")[0])
...
def update_Datastore(parsed: dict, date: str):
    """
...
def main():
    date = "Dec 2025"
    rank = 6 # No from the top row. (1 is top, 2 is second from top etc)
    ...
    pdf_path = download_pdf_by_rank(webpage_url, rank=rank)
